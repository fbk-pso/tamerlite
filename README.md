...
|------------------------------------------|-------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `search`                                 | `Optional[str]`   | Search algorithm to use. Supported values: `"astar"`, `"wastar"`, `"gbfs"`, `"bfs"`, `"dfs"`, `"ehc"`, `"novbfs_hg"`, `"novbfs_lg"`. Default: `"wastar"`. `"novbfs_hg"`/`"novbfs_lg"` (partitioned numeric novelty search, cost-maximizing/cost-minimizing final tie-break) always use an internal `hadd` heuristic, ignoring `heuristic`/`weight`. |
