...
pub fn multiqueue_search(ss: &mut SearchSpace, heuristics: Vec<(Heuristic, f64)>, timeout: Option<f32>) -> PyResult<(Option<Vec<(Option<String>, String, Option<String>)>>, HashMap<String, i32>)> {
        ...
        if let Some(current) = open.pop() {
            let successor_states;
            {
                ...
                if ss.goal_reached(&state, None)? {
                    metrics.insert("expanded_states".to_string(), states_expanded);
