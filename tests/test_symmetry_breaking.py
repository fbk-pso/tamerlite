...
def _required_object_action_problem(
    use_default: bool, use_argument: bool
...
def test_object_valued_initial_state_breaks_equivalence(
    ...
    problem, value_a, value_b = _required_object_action_problem(
        use_default, use_argument
    )
...
def test_symmetry_breaking_keeps_required_object_action() -> None:
    problem, _, _ = _required_object_action_problem(
        use_default=False, use_argument=True
    )
    planner = TamerLite(
        search=SearchParams(
            search="bfs",
