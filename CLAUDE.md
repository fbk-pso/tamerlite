...
**Relevance analysis: one fixpoint, two consumers.** `Encoder._compute_relevant_fluents(considered_actions)` computes the set of fluents that can actually affect search outcome as the least fixpoint of a backward slice: seeded from everything read by a precondition/effect-condition/goal/duration bound of a `considered_actions` action, then closed under "an effect's RHS matters only if the fluent it writes matters" (for every effect `f := expr`, once `f` is relevant every fluent `expr` reads becomes relevant too). The closure subsumes the older special-cased exclusion of an effect's own target fluent from its own right-hand side: increase/decrease effects desugar into a self-referencing assignment (`cost := cost + 1`), so a bookkeeping fluent like an accumulated cost counter only gets pulled in when it's already relevant — never from a bare self-reference, and never transitively through another bookkeeping fluent that is itself never seeded (e.g. `log := cost` where neither `log` nor `cost` is read by anything else). `considered_actions` (`Encoder.considered_actions`) is `relevant_actions` if relevance analysis narrowed it, else `applicable_actions` — the same action set the search will actually expand (`SearchSpace.get_successor_states` iterates `relevant_actions`). The result is computed unconditionally whenever the encoder is `full` and exposed as `Encoder.relevant_fluents`; it has two independent consumers with different soundness stories:

- **Dedup key** (unchanged behavior from before this was split out). `Encoder._dedup_relevant_fluents` is the gated view of `relevant_fluents`: `None` unless `relevant_equality` and (`not is_temporal` or `weak_equality`) — `is_temporal and not weak_equality` has no dedup at all (by design, to stay complete), so the reduction is never consulted there regardless of whether it was computed. When set, it's installed on `SearchSpace.dedup_relevant_fluents`, and every search algorithm in both cores keys its visited-state set/bloom filter on it instead of every fluent, restricting only the *dedup key*: the full state used for heuristic evaluation, effect application, and goal checking is untouched, so this can only make dedup recognize more states as duplicates, never change what an action does or which actions are applicable. It feeds both the classical (`not is_temporal`) dedup path and the temporal `weak_equality` dedup path through a single wrapper type (`WeakEqState` in both cores, plus `DedupKey` in Rust for the memory-bounded bloom-filter path) — one type suffices because `WeakEqState`'s extra `todo` (durative actions in progress) comparison is a no-op on the classical path: `todo` is provably always empty there (the initial state seeds it empty, and the only insertion is gated on the problem being temporal), so it doesn't need a *separate* `todo`-blind type. Rust always keys its visited-state sets on `WeakEqState` (`fluents=None` in the common case); a measured pass over `State`'s actual layout showed the wrapper's extra `fluents: Option<&[usize]>` field costs a few bytes per hash-set bucket against the ~3 KB a retained state already costs (dominated by `im::Vector`'s whole-chunk copy-on-write), so a second `Rc<State>`-keyed set to avoid it wasn't worth the duplicated branches across three search algorithms. Python keeps a bare-`State` fallback in `state_representation` for the `fluents=None, weak_equality=False` case instead, since there a `WeakEqState` is a real heap-allocated `dataclass` object (not a zero-cost Rust struct), so skipping the allocation is a genuine win, not a rounding error. Gated by `SearchParams.relevant_equality`/`MultiqueueParams.relevant_equality` (default `True`); turning it off leaves `Encoder._dedup_relevant_fluents`/`SearchSpace.dedup_relevant_fluents` at `None`, falling back to the pre-existing full-assignments dedup key -- `Encoder.relevant_fluents` itself is unaffected, since the heuristics need it regardless.

- **Heuristics.** `TamerLite._get_heuristic` (`engine.py`) builds `HFF`/`HAdd`/`HMax`/`HMaxExplicit` over `encoder.considered_actions` (not the full `applicable_actions` as before) and passes `encoder.relevant_fluents` as a `relevant_fluents` constructor argument, in both cores. Two distinct effects follow from this, with different soundness properties:
  - *Narrowing the operator set to `considered_actions` changes heuristic values* (it can only raise them, since a delete-relaxation operator built from a pruned-out action leaves the relaxation) and therefore `expanded_states`/`goal_depth` — this is intentional: the heuristic's relaxed planning graph now matches what the search can actually expand, instead of modeling a strictly larger transition relation.
  - *Restricting per-evaluation work to `relevant_fluents` is value-preserving* — every heuristic computes bit-identical values with or without it. `DeleteRelaxationHeuristic` never reads a non-relevant fluent's value: `_eval_core`'s three read sites (the per-fluent `costs` seeding loop, the `evaluate`/`internal_evaluate` sweeps over `_simple_/_complex_/_if_conds`, and `_repetitions`' `state.get_value(f)`) all only ever touch condition/goal leaves, which are exactly the fixpoint's seeds. `HMaxExplicit` *does* read effect RHS values (via `_possible_values`), which is exactly what the closure rule covers: pruning every effect whose target is irrelevant leaves every surviving effect's RHS relevant by construction. Both classes therefore prune effects targeting an irrelevant fluent (except each event's own synthetic progress-chaining effect, which is never in the relevant set since it addresses an index `>= len(fluent_types)` and must always fire), restrict their `internal_caching` memo key to the relevant-fluent projection of `state.assignments` instead of the whole vector, and seed their fixpoint's initial worklist from only the relevant fluents (`HMaxExplicit` additionally restricts its initial `assignments_changes`). `relevant_fluents=None` (the default on every constructor) takes every pre-existing code path verbatim, so callers other than `_get_heuristic` (e.g. `tests/test_engine.py::test_heuristic_values`, `mytests/`) are unaffected unless they opt in.

Because `tests/test_engine.py::check_metrics_equality` asserts identical `expanded_states`/`goal_depth` between the Python and Rust backends, any divergence between the two implementations of either consumer — dedup key or heuristic — fails that check; keep both cores in sync when touching either.
