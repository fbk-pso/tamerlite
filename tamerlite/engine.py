...
from tamerlite.core import entropy_dual_queue_search
    ...
    def _get_heuristic(self, params, heuristic, encoder, state_encoder):
        if params is None:
            ...
            h = (
                ...
                else params.heuristic if params.heuristic else "hff"
            )
            rl_params = params.rl_params

        cache_h = (
            False
            if params is None or params.cache_heuristic_in_state is None
            else params.cache_heuristic_in_state
        )
        ...
        if h == "custom":
            ...
            if rl_params.other_params.residual:
                internal_heuristic_cache = getattr(
                    rl_params.other_params, "internal_heuristic_cache", True
                )
                cache_heuristic_in_state = getattr(
                    rl_params.other_params, "cache_heuristic_in_state", False
                )
                hsym_params = SearchParams(
                    heuristic=rl_params.other_params.learning_heuristic,
                    internal_heuristic_cache=internal_heuristic_cache,
                    cache_heuristic_in_state=cache_heuristic_in_state,
                )
                heuristic_for_residual, _ = self._get_heuristic(
                    hsym_params, None, encoder, state_encoder
                )
            else:
            ...
            if h == "rl_heuristic":
                h = RLHeuristic(
                    state_encoder,
                    rl_params.model,
                    rl_params.model_class,
                    rl_params.other_params,
                    heuristic_for_residual,
                    cache_h,
                )
    ...
    def _solve(
        ...
        try:
            if self._params is not None and self._params.contains_rl():
                encoder, state_encoder, map_back_action_instance = get_encoders(
                    self._params.domain(), problem
                )
            else:
            ...
            if isinstance(self._params, MultiqueueParams):
                ...
                rl_search_params = SearchParams(
                    heuristic="rl_rank", weight=1, rl_params=self._params.rl_params, cache_heuristic_in_state=True
                )
                rank_policy, _ = self._get_heuristic(
                    rl_search_params, heuristic, encoder, state_encoder
                )
                plan, metrics = entropy_dual_queue_search(
